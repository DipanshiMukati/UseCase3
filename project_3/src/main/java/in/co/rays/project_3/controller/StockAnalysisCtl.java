...
public class StockAnalysisCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("stockSymbol"))) {
        	...
        	pass = false;
        	pass = false;
		} else if (!DataValidator.isAlphanumeric(request.getParameter("stockSymbol"))) {
			request.setAttribute("stockSymbol", "stockSymbol  must contains Alphanumeric Character only");
			pass = false;
		} else if (DataValidator.isTooLong(request.getParameter("stockSymbol"), 30)) {
			request.setAttribute("stockSymbol", "stockSymbol contain 30 characters");
			pass = false;
		} else if (!DataValidator.isStringLengthValid(request.getParameter("stockSymbol"), 3)) {
			request.setAttribute("stockSymbol", "stockSymbol  must be at least 5 characters long.");
			pass = false;
