...
public class SupplierCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		request.setAttribute("name", "  only 20 charector are allowed  ");
	...
	pass = false;
		...
		if (!OP_UPDATE.equalsIgnoreCase(request.getParameter("operation"))) {
			...
			else if (!DataValidator.isLong(request.getParameter("paymentTerm"))) {
				request.setAttribute("paymentTerm", "Only numbers are allowed.");
				pass = false;
			}
			 else if (DataValidator.isTooLong(request.getParameter("paymentTerm"), 15)) {
					request.setAttribute("paymentTerm", "  only 15 digits are allowed  ");
					pass = false;
				}
